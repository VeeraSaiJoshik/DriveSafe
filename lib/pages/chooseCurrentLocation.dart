...
gMap.LatLng getGPSPoints(
    double latitude, double longitude, int distance, double bearing) {
  const R = 6378.1;
...
//https://api.tomtom.com/traffic/services/5/incidentDetails?bbox=4.8854592519716675%2C52.36934334773164%2C4.897883244144765%2C52.37496348620152&fields=%7Bincidents%7Btype%2Cgeometry%7Btype%2Ccoordinates%7D%2Cproperties%7Bid%2CiconCategory%2CmagnitudeOfDelay%2Cevents%7Bdescription%2Ccode%2CiconCategory%7D%2CstartTime%2CendTime%2Cfrom%2Cto%2Clength%2Cdelay%2CroadNumbers%2CtimeValidity%2CprobabilityOfOccurrence%2CnumberOfReports%2ClastReportTime%7D%7D%7D&language=en-GB&categoryFilter=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C14&timeValidityFilter=present&key=
class weather {
  String time;
  String arrivalTime;
  double temp;
...
class chooseCurrentLocationScreen extends StatefulWidget {
  @override
  Location location;
  User user;
  ...
  chooseCurrentLocationScreen(this.location, this.user, this.currentLocation,
      this.isChooseScreen, this.userSettings);

  State<chooseCurrentLocationScreen> createState() =>
      _chooseCurrentLocationScreenState();
}

class _chooseCurrentLocationScreenState
    extends State<chooseCurrentLocationScreen>
    with SingleTickerProviderStateMixin {
  SearchingData searchByTextStatus = SearchingData.isSearching;
  String tomTomApiKey = "Insert Api Key";
  String openWeatherMapKey = "Insert Api Key";
  String googleApiKey = "Insert Api Key";
  String geoApifyApiKey = "Insert Api Key";
  int chosenId = -1;
  ...
  void calculationAccidnets() async {
    //longitude, latitude
    print("/traffic/services/5/incidentDetails?bbox=" +
    ...
    print(wantedHeight);
    print(findDistance1(
        northWest.latitude, northWest.longitude, mod.latitude, mod.longitude));
    markers = {};
    setState(() {
      markers;
    });
  ...
  void calculationWeather(List<PointLatLng> stopPoints) async {
    ...
    if (weatherWorthyPoints.length == 1) {
      weatherWorthyPoints.add(LatLng(destinationLocation!.position.latitude,
    ...
    for (int j = 0; j < arrivalsTimes.length; j++) {
      DateTime offsetTime = DateTime.now();

      DateTime destinationTime2 = offsetTime +
          arrivalsTimes[j].hour.hours +
          arrivalsTimes[j].minute.minutes +
          arrivalsTimes[j].day.days;
      offsetTime = DateTime.now().toUtc();
      ...
      for (int i = 0; i < timeList.length; i++) {
        ...
        DateTime currentDateTime =
            DateTime.parse(timeList[i]["dt_txt"] + ".000000Z").toUtc();
        ...
        Duration difference = currentDateTime.difference(destinationTime).abs();
        print(currentDateTime.toString());
        print(timeList[i]["visibility"]);
        print(destinationTime.toString());
        print(difference.inHours);
        if (difference.inHours <= 3.0) {
          print(timeList[i]);
          print(currentDateTime.toString());
          print(offsetTime.toString());
          print(destinationTime.toString());
                                                                  ...
                                                                  numberOfAccidents ==
                                                                      ...
                                                                      : Row(
                                                                          mainAxisAlignment:
                                                                              MainAxisAlignment.center,
                                                                          children: [
                                                                            Text("Number of " + atLastString + " : ",
                                                                                style: TextStyle(color: Colors.blue, fontWeight: FontWeight.w700, fontSize: textSize * 15)),
                                                                          ],
                                                                        ),
                                                                  Expanded(
                                                                    child:
